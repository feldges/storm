...
class StormOutlineGenerationModule(OutlineGenerationModule):
    """
    The interface for outline generation stage. Given an investment opportunity, collected information from knowledge
    curation stage, generate outline for the investment report.
    """
    ...
    def generate_outline(
        ...
        """
        Generates an outline for an investmetn report based on the specified investment opportunity and the information
        gathered during the knowledge curation stage. This method can optionally return both the
        final report outline and a draft outline if required.
        ...
        Args:
            ...
            old_outline (Optional[StormArticle]): An optional previous version of the report outline that can
            ...
            return_draft_outline (bool): A flag indicating whether the method should return both the final report
                outline and a draft version of the outline. If False, only the final report outline is returned.
                Defaults to False.
        ...
        article_with_outline_only = StormArticle.from_outline_str(
            opportunity=opportunity, outline_str=result.outline
        )
        article_with_draft_outline_only = StormArticle.from_outline_str(
            opportunity=opportunity, outline_str=result.old_outline
        )
...
class WriteOutline(dspy.Module):
    """Generate the outline for the investment report."""
    ...
    def forward(
        ...
        for turn in dlg_history:
            if (
                "opportunity you" in turn.agent_utterance.lower()
                or "opportunity you" in turn.user_utterance.lower()
            ):
                continue
            trimmed_dlg_history.append(turn)
        conv = "\n".join(
            [
                f"Investment Writer: {turn.user_utterance}\nExpert: {turn.agent_utterance}"
        ...
        with dspy.settings.context(lm=self.engine):
            if old_outline is None:
                old_outline = ArticleTextProcessing.clean_up_outline(
                    self.draft_page_outline(opportunity=opportunity).outline
                )
                if callback_handler:
            ...
            outline = ArticleTextProcessing.clean_up_outline(
                self.write_page_outline(
                    opportunity=opportunity, old_outline=old_outline, conv=conv
                ).outline
            )
            if callback_handler:
...
class WritePageOutline(dspy.Signature):
    """Write an outline for an investment report.
    Here is the format of your writing:
    1. Use "#" Title" to indicate section title, "##" Title" to indicate subsection title, "###" Title" to indicate subsubsection title, and so on.
    2. Do not include other information.
    3. Do not include investment opportunity name itself in the outline.
    """

    opportunity = dspy.InputField(prefix="The investment opportunity you want to write: ", format=str)
    outline = dspy.OutputField(prefix="Write the investment report outline:\n", format=str)
...
class NaiveOutlineGen(dspy.Module):
    ...
    def forward(self, opportunity: str):
        outline = self.write_outline(opportunity=opportunity).outline

        return dspy.Prediction(outline=outline)


class WritePageOutlineFromConv(dspy.Signature):
    """Improve an outline for an investment report. You already have a draft outline that covers the general information. Now you want to improve it based on the information learned from an information-seeking conversation to make it more informative.
    Here is the format of your writing:
    1. Use "#" Title" to indicate section title, "##" Title" to indicate subsection title, "###" Title" to indicate subsubsection title, and so on.
    2. Do not include other information.
    3. Do not include invesmtent opportunity name itself in the outline.
    """
    ...
    outline = dspy.OutputField(
        prefix='Write the investment report outline (Use "#" Title" to indicate section title, "##" Title" to indicate subsection title, ...):\n',
        format=str,
    )
