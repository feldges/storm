...
class ConvSimulator(dspy.Module):
    ...
    def forward(
        ...
        persona: The persona of the Investment writer.
        ...
        for _ in range(self.max_turn):
            user_utterance = self.wiki_writer(
                opportunity=opportunity, persona=persona, dialogue_turns=dlg_history
            ).question
            if user_utterance == "":
            ...
            expert_output = self.topic_expert(
                opportunity=opportunity, question=user_utterance, ground_truth_url=ground_truth_url
            )
        ...
        with dspy.settings.context(lm=self.engine):
            if persona is not None and len(persona.strip()) > 0:
                question = self.ask_question_with_persona(
                    opportunity=opportunity, persona=persona, conv=conv
                ).question
            else:
                question = self.ask_question(
                    opportunity=opportunity, conv=conv
                ).question
...
class AskQuestion(dspy.Signature):
    """You are an experienced Investment writer. You are chatting with an expert to get information for the investment opportunity you want to write a report on. Ask good questions to get more useful information relevant to the opportunity.
    When you have no more question to ask, say "Thank you so much for your help!" to end the conversation.
    Please only ask a question at a time and don't ask what you have asked before. Your questions should be related to the opportunity you want to write a report on.
    """

    opportunity = dspy.InputField(prefix="Investment opportunity you want to write: ", format=str)
    conv = dspy.InputField(prefix="Conversation history:\n", format=str)
    question = dspy.OutputField(format=str)


class AskQuestionWithPersona(dspy.Signature):
    """You are an experienced Investment writer and want to edit a specific page. Besides your identity as an Investment writer, you have specific focus when researching the opportunity.
    Now, you are chatting with an expert to get information. Ask good questions to get more useful information.
    When you have no more question to ask, say "Thank you so much for your help!" to end the conversation.
    Please only ask a question at a time and don't ask what you have asked before. Your questions should be related to the opportunity you want to write.
    """

    opportunity = dspy.InputField(prefix="Investment opportunity you want to write: ", format=str)
    persona = dspy.InputField(
        prefix="Your persona besides being an Investment writer: ", format=str
    )
    conv = dspy.InputField(prefix="Conversation history:\n", format=str)
    question = dspy.OutputField(format=str)
...
class TopicExpert(dspy.Module):
    ...
    def forward(self, opportunity: str, question: str, ground_truth_url: str):
        with dspy.settings.context(lm=self.engine, show_guidelines=False):
            # Identify: Break down question into queries.
            queries = self.generate_queries(opportunity=opportunity, question=question).queries
            queries = [
            ...
            if len(searched_results) > 0:
                ...
                try:
                    answer = self.answer_question(
                        opportunity=opportunity, conv=question, info=info
                    ).answer
...
class StormKnowledgeCurationModule(KnowledgeCurationModule):
    """
    The interface for knowledge curation stage. Given investment opportunity, return collected information.
    """
    ...
    def _get_considered_personas(self, opportunity: str, max_num_persona) -> List[str]:
        return self.persona_generator.generate_persona(
            opportunity=opportunity, max_num_persona=max_num_persona
        )
    ...
    def _run_conversation(
        ...
        Parameters:
            conv_simulator (callable): The function to simulate conversations. It must accept four
                parameters: `opportunity`, `ground_truth_url`, `persona`, and `callback_handler`, and return
                an object that has a `dlg_history` attribute.
            opportunity (str): The investment opportunity of conversation for the simulations.
            ground_truth_url (str): The URL to the ground truth data related to the conversation investment opportunity.
    ...
    def research(
        ...
        """
        Curate information and knowledge for the given investment opportunity

        Args:
            opportunity: investment opportunity of interest in natural language.
        ...
        if disable_perspective:
            ...
            considered_personas = self._get_considered_personas(
                opportunity=opportunity, max_num_persona=max_perspective
            )
